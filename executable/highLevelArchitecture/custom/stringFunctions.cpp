...
void realTimeConnectionSend(unsigned char *dataToList,unsigned char typeFeedback=DISABLE_FEEDBACK,unsigned long devId=DEV_ID){								// setting the data that we just got from the real time connection
	...
	while((REAL_TIME_SYNC_REGISTER!=(devId|(packetSequence&0xFFFFFF)))&&(!typeFeedback)){
		...
		during(realTimeConnectionSetList.size(),(unsigned long index){
			...
			_delay_ms(10);		//! this might be adjusted later
		});
	}
	...
	try{
		...
		const std::function<unsigned char*(unsigned char*)>jsonOperator[]={				// functional should be included so we can use lambda expression while passing variabels by ref
			...
			[&](unsigned char *subExecutable){											//& add event listener operator
				...
				while(VIRTUAL_CONTROLLER_EVENT_ADDRESS[eventAllocatorCounter++]&&(VIRTUAL_CONTROLLER_EVENT_ADDRESS[eventAllocatorCounter-1]!=eventListenerAddress));	// this should be initialized with 0s
